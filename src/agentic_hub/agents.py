...
AGENTS: dict[str, tuple[Path, Mode]] = {
    ...
    "codex": (HOME / ".agents/skills", "symlink"),
}
...
def _fanout_branch(agent: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"agent {agent!r} has no fan-out branch at {path}") from e
...
def _effective_mode(agent: str, name: str) -> Mode:
    ...
    try:
        has_marker = (
            FANOUT_MARKER in (catalog.SKILLS_DIR / name / "SKILL.md").read_text()
        )
    except OSError:
        has_marker = False
...
def _spliced_status(agent: str, name: str, tp: Path) -> Status:
    if tp.is_symlink():
        ...
        # conflict.
        return (
            "stale" if _is_ours_symlink(tp, catalog.SKILLS_DIR / name) else "conflict"
        )
    if not tp.is_dir():
...
def _install_spliced(agent: str, name: str, sp: Path, tp: Path) -> bool:
    ...
    if skill_md.exists() and not is_our_spliced_skill(skill_md):
        log.warning(
            "skip %s/%s: %s exists and wasn't generated by agentic-hub",
            agent,
            name,
            skill_md,
        )
